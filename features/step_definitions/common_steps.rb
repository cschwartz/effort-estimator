...
When(/^I choose to edit the (category|parameter|project|estimation option) "([^"]*)"$/) do |resource_type, resource_title|
...
When(/^I choose to delete the (category|parameter|project|estimation option) "([^"]*)"$/) do |resource_type, resource_title|
  container_class = resource_type.parameterize(separator: '_').pluralize
...
Then(/^I should see the (category|parameter|project|estimation option) "([^"]*)"$/) do |resource_type, resource_title|
  container_id = resource_type.parameterize(separator: '_').pluralize
  within("##{container_id}") do
...
Then(/^I should not see the (category|parameter|project|estimation option) "([^"]*)"$/) do |resource_type, resource_title|
  container_id = resource_type.parameterize(separator: '_').pluralize
  within("##{container_id}") do
...
When(/^I create the (category|parameter|project|estimation option)$/) do |resource_type|
...
When(/^I update the (category|parameter|project|estimation option)$/) do |resource_type|
  display_name = resource_type.split.map.with_index { |word, i| i.zero? ? word.capitalize : word }.join(' ')
  button_text = "Update #{display_name}"
  click_button button_text
end
